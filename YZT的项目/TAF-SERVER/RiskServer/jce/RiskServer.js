...
Risk.RiskInterImp.prototype.onDispatch = function (current, FuncName, BinBuffer) { 
    switch (FuncName) {
        case 'queryUserAuth' : {
            if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                ...
                var stReq = wup.readStruct("stReq", Risk.UserAuthReq);
                var stRsp = wup.readStruct("stRsp", Risk.UserAuthRsp, new Risk.UserAuthRsp());
            } else {
                var is = new TafStream.JceInputStream(BinBuffer);
                var stReq = is.readStruct(1, true, Risk.UserAuthReq);
                var stRsp = is.readStruct(2, false, Risk.UserAuthRsp);
            }
            ...
            this.queryUserAuth(current, stReq, stRsp);
        ...
        case 'queryUserRisk' : {
            if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                ...
                wup.wupVersion = current.getRequestVersion();
                wup.decode(BinBuffer);
                var userName = wup.readString("userName");
                var userRisk = wup.readStruct("userRisk", Risk.UserRisk, new Risk.UserRisk());
            } else {
                var is = new TafStream.JceInputStream(BinBuffer);
                var userName = is.readString(1, true);
                var userRisk = is.readStruct(2, false, Risk.UserRisk);
            }
            current.sendResponse = function (_ret, userRisk) {
                if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                    var wup = new TafStream.UniAttribute();
                    wup.wupVersion = current.getRequestVersion();
                    wup.writeInt32("", _ret);
                    wup.writeStruct("userRisk", userRisk);

                    current.doResponse(wup.encode());
                } else {
                    var os = new TafStream.JceOutputStream();
                    os.writeInt32(0, _ret);
                    os.writeStruct(2, userRisk);

                    current.doResponse(os.getBinBuffer());
                }
            }

            this.queryUserRisk(current, userName, userRisk);
        ...
        case 'setUserRisk' : {
            if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                ...
                wup.wupVersion = current.getRequestVersion();
                wup.decode(BinBuffer);
                var userRisk = wup.readStruct("userRisk", Risk.UserRisk);
            } else {
                var is = new TafStream.JceInputStream(BinBuffer);
                var userRisk = is.readStruct(1, true, Risk.UserRisk);
            }
            ...
            this.setUserRisk(current, userRisk);
...
Risk.RiskInterImp.prototype.queryUserAuth = function (current, stReq, stRsp) {
    //TODO:
    assert.fail('queryUserAuth function not implemented');
}

Risk.RiskInterImp.prototype.queryUserRisk = function (current, userName, userRisk) {
    //TODO:
    assert.fail('queryUserRisk function not implemented');
}

Risk.RiskInterImp.prototype.setUserRisk = function (current, userRisk) {
    //TODO:
    assert.fail('setUserRisk function not implemented');
}
